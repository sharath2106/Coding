...
public class StringsEndingWithPattern {
  ...
  static boolean endsWith(String str, String pat) {
    ...
    while (patLen >= 0) {
      ...
      // with pattern
      if (pat.charAt(patLen) != str.charAt(strLen)) return false;
      patLen--;
      strLen--;
    }
  ...
  static int countOfStrings(String pat, int n, String sArr[]) {
    int count = 0;
    for (int i = 0; i < n; i++) {

      // If current string ends with
      // the given pattern
      if (endsWith(sArr[i], pat)) count++;
    }
    return count;
  }
  ...
  public static void main(String[] args) {
    String pat = "ks";
    int n = 4;
    String sArr[] = {"geeks", "problems.Day0.geeksforgeeks", "games", "unit"};
