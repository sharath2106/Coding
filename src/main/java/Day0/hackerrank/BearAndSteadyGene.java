...
public class BearAndSteadyGene {
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < N; i++) count.put(s.charAt(i), count.get(s.charAt(i)) + 1);

    if (count.get('A') < 0) count.put('A', 0);

    if (count.get('C') < 0) count.put('C', 0);

    if (count.get('G') < 0) count.put('G', 0);

    if (count.get('T') < 0) count.put('T', 0);
